        ...
        <div  v-show="printView ? false : true">
            ...
            <el-container class="container-row">
                ...
                <div class="input-field">
                    ...
                    <el-button type="primary" style="font-size: 16px; color: #000;" :disabled="isPrint" @click="print">打印</el-button>
                </div>
            </el-container>
    ...
    methods: {
        ...
        deleteEdit(row) {
            this.$confirm(`删除有风险，是否需要删除？`, "警告提示", {
                ...
                .then(() => {
                    getAxios("deletematerial", { _id: row._id, name: row.name, typeName: row.typeName, num: row.num, type: 2 }).then(res => {
                        if (res.state == 200) {
                            this.message(res.msg, 'success')
                            // 重新渲染页面
                            this.RenderList()
        ...
        RenderList() {
            getAxios("materialgood", {
                type: 4, pageIndex: this.pageIndex,
                pageSize: this.pageSize
            }).then(res => {
                if (res.state == 200) {
                    let list = []
